...
export function DiskSpaceWidget({ stats }: DiskSpaceWidgetProps) {
  ...
  // Only show charts when there's meaningful variance to visualise
  const showCharts = pieData.length > 1 || stats.duplicate_files > 0 || trendData.length > 0
    ...
    >
      ...
      >
        <span className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
          {t('cleanup.diskSpace.title', 'Disk-Analyse')}
        </span>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatBox
            label={t('cleanup.diskSpace.totalFiles', 'Dateien gesamt')}
            value={stats.total_files.toLocaleString()}
          />
          <StatBox
            label={t('cleanup.diskSpace.totalSize', 'Gesamtgröße')}
            value={formatBytes(stats.total_size_bytes)}
          />
          <StatBox
            label={t('cleanup.diskSpace.duplicates', 'Duplikate')}
            value={stats.duplicate_files.toLocaleString()}
            color="var(--warning)"
          />
          <StatBox
            label={t('cleanup.diskSpace.savings', 'Mögliche Einsparung')}
            value={formatBytes(stats.potential_savings_bytes)}
            color="var(--success)"
          />
        </div>

        {/* Charts — only when there's meaningful data to show */}
        {showCharts && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              <h4 className="text-xs font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>
                {t('cleanup.diskSpace.byFormat', 'Nach Format')}
              </h4>
              {pieData.length > 0 ? (
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2">
                {pieData.map((entry) => (
                  <div key={entry.name} className="flex items-center gap-1 text-[10px]">
                    <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: getFormatColor(entry.name) }} />
                    <span style={{ color: 'var(--text-secondary)' }}>{entry.name} ({entry.count})</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            >
              <h4 className="text-xs font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>
                {t('cleanup.diskSpace.storageBreakdown', 'Speicheraufteilung')}
              </h4>
              <ResponsiveContainer width="100%" height={140}>
                <BarChart data={barData} layout="vertical">
                  ...
                  <Tooltip
                    formatter={((value: number | undefined, name: string | undefined) => [
                      formatBytes(value ?? 0),
                      name === 'total'
                        ? t('cleanup.diskSpace.uniqueData', 'Einzigartig')
                        : t('cleanup.diskSpace.duplicateData', 'Duplikat'),
                    ]
                      // eslint-disable-next-line @typescript-eslint/no-explicit-any
                      ) as any}
                    contentStyle={{
              ...
              <div className="flex gap-4 mt-2 justify-center">
                <div className="flex items-center gap-1.5 text-[10px]">
                  ...
                  <span style={{ color: 'var(--text-secondary)' }}>{t('cleanup.diskSpace.uniqueData', 'Einzigartig')}</span>
                </div>
                <div className="flex items-center gap-1.5 text-[10px]">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: 'var(--warning)' }} />
                  <span style={{ color: 'var(--text-secondary)' }}>{t('cleanup.diskSpace.duplicateData', 'Duplikat')}</span>
                </div>
              </div>
            </div>
            ...
            >
              <h4 className="text-xs font-semibold mb-2" style={{ color: 'var(--text-secondary)' }}>
                {t('cleanup.diskSpace.trends', 'Bereinigungs-Trends (30T)')}
              </h4>
              {trendData.length > 0 ? (
                <ResponsiveContainer width="100%" height={140}>
                ...
                <div className="flex items-center justify-center h-[140px] text-xs" style={{ color: 'var(--text-muted)' }}>
                  {t('cleanup.diskSpace.noTrends', 'Noch kein Bereinigungsverlauf')}
                </div>
              )}
            </div>
          </div>
