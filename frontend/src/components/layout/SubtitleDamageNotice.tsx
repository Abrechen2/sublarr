/**
 * One-time notice about the HI-removal data-loss bug (see CHANGELOG 1.6.6).
 *
 ...
 *
 * This is a data-integrity apology, not a feature announcement — hence its own
 * modal rather than a WhatsNew entry. Shown once, then remembered.
 */
      ...
      >
        ...
        <div className="px-4 py-3 space-y-3 overflow-auto text-xs text-primary leading-relaxed">
          ...
          <div className="rounded-md p-3 bg-page border border-border font-mono text-[11px]">
            ...
            <div>darf ich dich <span style={{ color: 'var(--accent)' }}>um</span> eine Sache bitten?</div>
          ...
          <p className="text-muted">{t('damage_notice.backups')}</p>
          <p className="text-muted">{t('damage_notice.fixed')}</p>
          <p className="text-muted">{t('damage_notice.next')}</p>
        </div>
