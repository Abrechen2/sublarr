...
#: Both spellings of a line break. A guard that knows only one of them is a
#: guard the model walks around: 459 of prod's 254 196 memory entries carried
#: an invented SOFT break on 2026-09-03, still growing daily, while invented
#: hard breaks were already down to zero.
_BREAKS = (_HARD_BREAK, _SOFT_BREAK)


def strip_invented_hard_breaks(source_lines: list[str], translated_lines: list[str]) -> list[str]:
    """Drop line breaks the model added where the source had none.
    ...
    Only lines whose source has no break **in either spelling** are touched, so
    a break the subtitle actually asked for survives untouched — including one
    the model merely re-spelled, where the source wanted a hard break and the
    translation carries a soft one. An inner break becomes a single space (it
    was standing between two words); a trailing one simply goes. Lists of
    differing length are returned unchanged — pairing would be guesswork, and
    the caller already treats that as a failure.
    """
    ...
    for source, translated in zip(source_lines, translated_lines, strict=True):
        source_has_break = any(marker in source for marker in _BREAKS)
        translated_has_break = any(marker in translated for marker in _BREAKS)
        if source_has_break or not translated_has_break:
