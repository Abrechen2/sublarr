"""ChatGPT (OpenAI) translation backend — OpenAI-compatible API (native)."""
...
class ChatGPTBackend(LLMBackend):
    """OpenAI ChatGPT translation backend via chat/completions API."""
    ...
    cost_per_1m_tokens_in = Decimal("0.15")
    cost_per_1m_tokens_out = Decimal("0.60")
    ...
    def __init__(self, api_key: str, model: str | None = None, **_: Any) -> None:
        if not api_key:
            raise ValueError("ChatGPTBackend: api_key is required")
    ...
    def _build_request(self, messages: list[dict], max_tokens: int) -> dict:
        """OpenAI native chat/completions shape."""
        return {
            "model": self._model,
            "messages": messages,
