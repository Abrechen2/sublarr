...
class SocketIOLogHandler(logging.Handler):
    ...
    DB-internal error details (column names, SQL function signatures) are
    stripped before emission so that schema information never leaks to clients.
    """

    # Prefixes from DB drivers that reveal schema details
    _DB_ERROR_PREFIXES = (
        "psycopg2.",
        "sqlalchemy.",
        "(psycopg2",
    ...
    def _sanitize(msg: str) -> str:
        """Replace DB-internal error detail with a generic message."""
        lower = msg.lower()
        if any(p.lower() in lower for p in SocketIOLogHandler._DB_ERROR_PREFIXES):
            # Keep timestamp/level prefix (everything before the first ']'), drop the detail
            bracket = msg.find("]")
            prefix = msg[: bracket + 1] + " " if bracket != -1 else ""
            return prefix + "Database error (details suppressed)"
        return msg
