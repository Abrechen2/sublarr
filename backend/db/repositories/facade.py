"""Module-level convenience functions over the repository classes.

Extracted from db/repositories/__init__.py to keep the package entry point
focused on re-exports and public API surface. Each function is a thin
wrapper that instantiates the relevant Repository and delegates a single
method call — the legacy signature that predates the repository pattern.
"""
