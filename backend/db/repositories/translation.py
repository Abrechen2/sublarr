...
The translation-memory cache (lookup/store/clear/stats) lives in the
sibling ``translation_cache.py`` as ``_TranslationCacheMixin`` and is
composed into this class via multiple inheritance.
"""
...
from db.repositories.translation_cache import (  # noqa: F401 — re-exported for back-compat
    _FUZZY_SCAN_LIMIT,
    _TranslationCacheMixin,
)

logger = logging.getLogger(__name__)


class TranslationRepository(BaseRepository, _TranslationCacheMixin):
