...
def upgrade() -> None:
    ...
    if dialect == "postgresql":
        op.drop_constraint(_PG_OLD_UQ, _TABLE, type_="unique")
        ...
        with op.batch_alter_table(_TABLE, naming_convention=_SQLITE_NAMING) as batch:
            try:
                batch.drop_constraint(_SQLITE_OLD_UQ, type_="unique")
            except Exception:  # noqa: BLE001 — a fresh DB never had the old one
                pass
