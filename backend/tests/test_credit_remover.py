...
def test_role_marker_translator_detected():
    ...
    assert (
        _is_credit_line(
            "Translation by (Translator) John Smith", total_end_ms=120_000, event_start_ms=10_000
        )
        is True
    )
...
def test_role_marker_qc_detected():
    ...
    assert (
        _is_credit_line("Quality Check (QC)", total_end_ms=120_000, event_start_ms=10_000) is True
    )
...
def test_prefix_credits_colon_detected():
    ...
    assert (
        _is_credit_line("Credits: Some Names", total_end_ms=120_000, event_start_ms=10_000) is True
    )
...
def test_prefix_translation_colon_detected():
    ...
    assert (
        _is_credit_line("Translation: Jane Smith", total_end_ms=120_000, event_start_ms=10_000)
        is True
    )
...
def test_duration_guard_question_mark_not_removed():
    ...
    assert (
        _is_credit_line("Are you okay?", total_end_ms=1_440_000, event_start_ms=1_430_000) is False
    )
...
def test_duration_outside_credits_region_ignored():
    ...
    assert (
        _is_credit_line("The door creaked loudly", total_end_ms=1_440_000, event_start_ms=5_000)
        is False
    )
...
def test_isolated_name_three_words_not_detected():
    ...
    assert (
        _is_credit_line("John Michael Smith", total_end_ms=120_000, event_start_ms=5_000) is False
    )
...
def test_normal_dialogue_not_removed():
    ...
    assert (
        _is_credit_line("I'll be back tomorrow.", total_end_ms=120_000, event_start_ms=30_000)
        is False
    )
