"""DeepSeekBackend tests — mocked requests.post."""
...
def _deepseek(api_key="sk-test", model="deepseek-chat"):
    from translation.deepseek import DeepSeekBackend

    return DeepSeekBackend(api_key=api_key, model=model)
...
def test_construction_requires_api_key():
    ...
    with pytest.raises(ValueError, match="api_key"):
        DeepSeekBackend(api_key="", model="deepseek-chat")
...
def test_default_model_and_prices():
    ...
    assert b.cost_per_1m_tokens_in == Decimal("0.14")
    assert b.cost_per_1m_tokens_out == Decimal("0.28")
    assert b.supports_glossary is True
    assert b.supports_batch is True


def test_build_request_preserves_openai_shape():
    """DeepSeek accepts OpenAI chat/completions format — no reshape."""
    b = _deepseek()
    messages = [
...
def test_call_api_uses_bearer_auth():
    ...
    with patch("translation.deepseek.requests.post") as mock_post:
...
def test_cost_calculation_matches_price_sheet():
    ...
    b = _deepseek()
    # 1000 * 0.14/1M + 500 * 0.28/1M = 0.00014 + 0.00014 = 0.00028 = 280 micro_usd
...
def test_deepseek_coder_model_also_priced():
    b = _deepseek(model="deepseek-coder")
    assert b.cost_per_1m_tokens_in == Decimal("0.14")
    assert b.cost_per_1m_tokens_out == Decimal("0.28")
...
def test_end_to_end_with_mock(app, reset_conc):
    ...
    get_concurrency().register("deepseek", 2)
    b = _deepseek()
