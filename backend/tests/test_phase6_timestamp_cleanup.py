...
def test_whisper_queue_passes_datetime_to_update(monkeypatch):
    ...
    with (
        patch("whisper.queue.update_whisper_job", fake_update_whisper_job),
        patch("whisper.queue.create_whisper_job"),
        patch("whisper.queue.select_audio_track", side_effect=RuntimeError("no audio")),
        patch("whisper.queue.get_audio_track_by_index", side_effect=RuntimeError("no audio")),
    ):
