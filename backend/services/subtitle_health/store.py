...
def persist_scan_result(result, scanner_version: int = 1) -> None:
    ...
    try:
        ...
        for issue in result.issues:
            ...
            db.session.add(row)
            # Flush to assign the autogenerated PK, then backfill it onto the
            # in-memory issue so the scan API response carries the finding id
            # the fix endpoint requires.
            db.session.flush()
            issue.id = row.id
        db.session.commit()
    except Exception:
