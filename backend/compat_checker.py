...
from compat_checker_data import _ISO_639_1, _ISO_639_2, _LANGUAGE_NAMES
...
def _get_sub_basename(subtitle_filename: str) -> str:
    ...
    for i in range(len(parts) - 1, 0, -1):
        ...
        if (
            part_lower in modifiers
            or _is_valid_iso639(part_lower)
            or _is_valid_bcp47_or_name(part_lower)
        ):
            strip_count += 1
        else:
            break
