...
- **Scene-detection markers** — when PySceneDetect is installed, scene-cut boundaries paint as thin amber vertical lines on the waveform, anchored to percentage-of-duration so they stay aligned across zoom. Strictly informational in this release — they help users avoid setting a cue boundary across a hard cut, but they don't yet alter snap behaviour. Snap-to-scene is a future follow-up.
...
### Deferred
- **ASS karaoke syllable overlay (Tier 2)** — backend syllable parsing + frontend per-syllable tick rendering on the waveform. Deferred to a follow-up; the current release displays the cue, not its syllable structure.
- **Snap-to-scene-cut** — uses the new scene markers as snap targets in addition to keyframes. Trivial extension to `snap.ts` once we want it.
