...
### Fixed
- **Automatic database migrations were silently skipped.** The migration
  graph had accumulated nine branch heads, so the automatic
  `alembic upgrade head` on startup failed with "Multiple head revisions are
  present" (logged only as a non-fatal warning). A merge revision unifies the
  graph so pending migrations apply again on Alembic-tracked installs.
