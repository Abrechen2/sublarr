...
- A provider needs **50 searches after the reset** before the
  delivers-nothing verdict may fire, so expect a quiet window before a
  genuinely dead provider is called out.
- `i198_reset_counters` deliberately leaves `consecutive_failures` alone,
  so a provider that was broken *before* the upgrade and is fixed *by* it
  can read `healthy: false, status_reason: "consecutive_failures"` on
  purely pre-fix history. It is cosmetic and clears itself the moment that
  provider completes one successful search — the dispatch path only skips
  auto-disabled providers, never on the streak. Reported by @Svnow132.

The same applies retroactively to **1.13.4**, whose `tm1_strip_breaks`
rewrote translation-memory rows in place: it was described in that
release's notes but not flagged as data-changing, which is the gap these
notes close.
...
- **The translation memory drops entries that were never translations.** The
  ...
  `en -> en` whatever the source actually was. None of those rows are harmless:
  on the reference install all 5 302 differ from their source, and they turn out
  to be French source text stored against German output. Each was served
  verbatim on a cache hit, putting German into an English subtitle. A migration
  removes them on upgrade. Nothing is lost that matters — a cache entry is
  regenerated by the next translation, and no subtitle content lives only here.

  The quality retry was not the only writer, which is why the migration alone
  was not enough: on our own production instance it cleaned the table at the
  upgrade, and the wanted-search and embedded-stream paths refilled it with
  1 221 fresh `en -> en` rows within two days. Those paths are closed by the
  same-language fix above, so this migration is now a one-time cleanup rather
...
- **`_resolve_trash_dir` rejects relative-path traversal (audit G3 + Gemini R2)** — An absolute `remux_trash_dir` on `/etc`, `/proc`, `/sys`, `/dev`, `/run`, `/boot`, `/root`, `/var/log`, `/var/run`, `/var/lib`, `/usr`, `/lib`, `/lib64`, `/sbin`, `/bin` falls back to `<media>/.sublarr`. Relative settings like `../../../etc` used to be `os.path.join`-ed and returned unchecked; the resolver now `is_safe_path`-validates the joined path.
...
- **Defence-in-depth boundary on `POST /wanted/<id>/extract` (audit E1-1)** — `_validate_extract_target` now runs `is_safe_path` against `wanted_items.file_path` before forking ffmpeg/mkvmerge, preventing an attacker-controlled DB row from steering subprocess at `/etc/anything`.
...
- **`_make_backup` no longer falls back to a sibling `.bak` (audit G7)** — When the configured trash dir was unreachable (read-only mount, missing parent), the helper used to write `<original>.bak` next to the original. That hid configuration problems behind a "recovery copy next to your video" UX surprise. Failure now raises `RemuxError` and the caller aborts the destructive remux swap.
