...
must_haves:
  truths:
    ...
    - "Provider config fields are declared per-provider class (not hardcoded in ProviderManager switch/case)"
    ...
    - "A bad plugin (syntax error, missing dependency) does not prevent the app from starting"
  artifacts:
...
<task type="auto">
  <name>Task 1: Extend SubtitleProvider ABC with declarative config_fields and refactor ProviderManager</name>
  <files>
    backend/providers/base.py
    backend/providers/__init__.py
    backend/providers/animetosho.py
  ...
  <action>
    1. In `backend/providers/base.py`, add a `config_fields` class attribute to `SubtitleProvider`:
       ```python
    ...
    2. In `backend/providers/__init__.py`:
       ...
       e. In `_init_providers()`, after importing built-in providers, call `_load_plugins()` which imports from the plugin manager (deferred to Task 2 -- for now, add the method stub that does nothing if plugin manager is not available).

    3. Update the 4 existing built-in providers to declare their own `config_fields`:
       - `animetosho.py`: `config_fields = []` (no auth needed), `rate_limit = (50, 30)`, `timeout = 20`, `max_retries = 2`
       - `jimaku.py`: `config_fields = [{"key": "jimaku_api_key", "label": "API Key", "type": "password", "required": True}]`, `rate_limit = (100, 60)`, `timeout = 30`, `max_retries = 2`
       - `opensubtitles.py`: `config_fields = [{"key": "opensubtitles_api_key", ...}, {"key": "opensubtitles_username", ...}, {"key": "opensubtitles_password", ...}]`, `rate_limit = (40, 10)`, `timeout = 15`, `max_retries = 3`
       - `subdl.py`: `config_fields = [{"key": "subdl_api_key", "label": "API Key", "type": "password", "required": True}]`, `rate_limit = (30, 10)`, `timeout = 15`, `max_retries = 2`

    Keep the exact same key names as currently used in the Pydantic Settings model so the existing config cascade continues to work. The `register_provider` decorator remains in `__init__.py`; built-in providers still use it.
  </action>
  <verify>
    - `cd backend && python -c "from providers.base import SubtitleProvider; assert hasattr(SubtitleProvider, 'config_fields')"`
    - `cd backend && python -c "from providers import _PROVIDER_CLASSES; assert len(_PROVIDER_CLASSES) >= 4; print([c.config_fields for c in _PROVIDER_CLASSES.values()])"`
    - `cd backend && python -m pytest tests/ -x -q --timeout=30` (existing tests still pass)
  </verify>
  <done>
    SubtitleProvider ABC has config_fields, rate_limit, timeout, max_retries class attributes. All 4 built-in providers declare their own values. ProviderManager reads from class attributes instead of hardcoded switch/case. Existing behavior is preserved.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create plugin discovery system, manifest validation, and DB config storage</name>
  <files>
    backend/providers/plugins/__init__.py
    backend/providers/plugins/loader.py
    backend/providers/plugins/manifest.py
  ...
  <action>
    ...
    3. `backend/providers/plugins/loader.py`:
       - Define `discover_plugins(plugins_dir: str, existing_names: set[str]) -> tuple[dict[str, type], list[dict]]`:
         ...
         - Wraps each module load in try/except to prevent bad plugins from crashing the app.
         - For each loaded module, inspects members for SubtitleProvider subclasses.
    ...
    8. In `backend/db/__init__.py`, no schema changes needed -- `config_entries` table already exists and is used for plugin config storage.
  </action>
  <verify>
  ...
  <done>
    PluginManager discovers .py files from plugins_dir, validates SubtitleProvider subclasses, registers them in _PROVIDER_CLASSES with is_plugin=True. Plugin config reads/writes to config_entries with namespaced keys. Bad plugins log errors but don't crash the app. Settings has plugins_dir field.
  </done>
</task>

<task type="auto">
  <name>Task 3: Create plugin management API endpoints and register blueprint</name>
  <files>
    backend/routes/plugins.py
    backend/routes/__init__.py
    backend/app.py
  ...
  <action>
    1. Create `backend/routes/plugins.py` with Blueprint `bp = Blueprint("plugins", __name__, url_prefix="/api/v1")`:
    ...
    2. Register the plugins blueprint in `backend/routes/__init__.py`: add `from routes.plugins import bp as plugins_bp` and include it in the blueprint list.

    3. In `backend/app.py`, in `create_app()`, after `init_db()` and before `register_blueprints()`, add plugin initialization:
       ```python
    ...
    4. Add `backend/requirements.txt` entries for new dependencies (needed by Plan 04-06 providers but best to add now):
       ```
  ...
  <done>
    Plugin management API is accessible at /api/v1/plugins (list), /api/v1/plugins/reload (POST), /api/v1/plugins/<name>/config (GET/PUT). Blueprint is registered in the app factory. Plugin discovery runs on startup, creating the plugins_dir if it doesn't exist. New dependencies added to requirements.txt.
  </done>
</task>
