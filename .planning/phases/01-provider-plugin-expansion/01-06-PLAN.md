...
<task type="auto">
  <name>Task 2: Implement LegendasDivx provider (Portuguese subtitle scraping with session auth)</name>
  ...
  <action>
    ...
    **_login() internal method:**
    ...
    6. Track daily search count in an instance variable `_search_count`.

    **_ensure_authenticated() internal method:**
    - Check if session cookies are still valid by looking for a logged-in indicator.
    - If cookies expired (detected via 302 redirect to login page), call `_login()`.
    - If `_search_count >= 140` (safety margin below 145 limit), raise ProviderRateLimitError with message about daily limit.
    ...
    **Critical considerations from research:**
    - 145 searches/day limit is strict. Track daily count with `_search_count` reset daily (compare date).
  ...
  <done>
    LegendasDivxProvider is registered, handles session-based auth with lazy login, scrapes Portuguese subtitles with daily rate limit tracking (145/day safety margin), extracts from RAR/ZIP archives, and handles session expiry with re-authentication.
  </done>
</task>
