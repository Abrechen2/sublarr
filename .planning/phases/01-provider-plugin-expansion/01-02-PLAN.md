  ...
  <action>
    ...
    2. In `frontend/src/pages/Settings.tsx`:
       a. In the `ProviderCard` component, add a health stats section below the existing stats row. Display:
          - Success rate as a percentage bar (green if >80%, yellow if >50%, red if <50%)
          - Average response time (formatted as "XXms")
          - Last response time
          - Auto-disabled badge if provider is auto-disabled (with "disabled until" time)
          - Consecutive failures count (only show if > 0, with warning styling)
       b. Add a small "Re-enable" button that appears when a provider is auto-disabled. It calls `POST /providers/<name>/enable` (add this mutation to useApi hooks if not already present, or use a direct fetch call).
       c. Use Tailwind classes consistent with the existing teal theme. Success rate bar: h-1.5 rounded-full with bg-emerald-500/bg-amber-500/bg-red-500.
       d. Keep the UI compact -- health stats should fit in 1-2 rows below the existing download/cache stats.
  </action>
  <verify>
    - `cd frontend && npx tsc --noEmit` (TypeScript compiles without errors)
    - `cd frontend && npm run lint` (no lint errors)
    - Visual: Run `npm run dev` and check the Providers tab in Settings -- each provider should show success rate, response time, and health indicators.
  </verify>
  <done>
    Frontend types include provider health stats (response time, auto-disable status). Settings page Providers tab shows per-provider success rate bar, response times, consecutive failures, and auto-disable badge with re-enable button.
  </done>
</task>
...
- Frontend displays success rate bar, response times, and auto-disable status per provider
