...
<task type="auto">
  <name>Task 2: Implement Napisy24 provider (hash-based POST API for Polish subs)</name>
  <files>backend/providers/napisy24.py</files>
  <action>
  ...
  <verify>
    ...
    - `cd backend && python -c "from providers import _PROVIDER_CLASSES; assert 'napisy24' in _PROVIDER_CLASSES; print('Napisy24 registered')"`
    - `cd backend && python -m pytest tests/ -x -q --timeout=30`
  </verify>
  <done>
    Napisy24Provider searches Polish subs via file hash with POST API, extracts from ZIP archives, and handles errors gracefully.
  </done>
</task>

<task type="auto">
  <name>Task 3: Implement Whisper-Subgen provider (external ASR service delegation)</name>
  <files>backend/providers/whisper_subgen.py</files>
  <action>
    ...
    **Important constraints (from research):**
    ...
    - If ffmpeg is not available, raise ProviderError with message "ffmpeg not found" rather than crashing.
  </action>
  <verify>
    ...
    - `cd backend && python -c "from providers import _PROVIDER_CLASSES; assert 'whisper_subgen' in _PROVIDER_CLASSES; print('WhisperSubgen registered')"`
    - `cd backend && python -m pytest tests/ -x -q --timeout=30`
  </verify>
  <done>
    WhisperSubgenProvider delegates transcription to an external Subgen instance, returns low-score placeholder in search (last resort), extracts audio via ffmpeg, and handles missing ffmpeg gracefully.
  </done>
</task>
