...
NOTE: SeriesDetail.tsx is also modified by Plan 02 (comparison/sync buttons). Both plans are Wave 2 and touch this file. To avoid conflicts:
- Plan 02 adds `comparisonPaths` and `syncFilePath` state, Compare and Sync buttons.
- Plan 03 adds `healthCheckPath` state, Health button and HealthBadge/HealthCheckPanel.
- Plan 03 should READ SeriesDetail.tsx as modified by Plan 02 (if already executed) or handle clean addition if not yet modified.
- The integration point is the episode action buttons row -- each plan adds its own buttons independently.
  </action>
  <verify>
`cd frontend && npx tsc --noEmit 2>&1 | head -20` -- zero TypeScript errors.
