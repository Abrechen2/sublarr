...
NOTE on scope: Task 1 creates 6 model files, but these are boilerplate ORM definitions transcribed from the existing SCHEMA DDL constant. Each model is a straightforward column-to-mapped_column translation. Context usage is significantly lower than the file count suggests because there is no business logic -- just declarative schema definitions.
</objective>
