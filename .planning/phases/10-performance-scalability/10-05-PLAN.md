...
Run `cd Z:/CC/Sublarr/backend && python -c "from db import _db_lock, get_db; print(type(_db_lock).__name__)"` -- should print "_NoOpLock".
  ...
  <done>App factory initializes Flask-SQLAlchemy + Alembic + cache + queue. Config has new database/Redis settings. db/__init__.py uses SQLAlchemy session. _db_lock is no-op. transaction_manager uses SQLAlchemy.</done>
</task>

<task type="auto">
  <name>Task 2: Redirect all db/ domain modules to repository layer</name>
  <files>
    backend/db/config.py
    backend/db/blacklist.py
    backend/db/cache.py
  ...
  <action>
Rewrite each of the 14 db/ domain modules to be thin wrappers that delegate to their repository counterpart. This preserves ALL existing import paths (`from db.jobs import create_job` still works) while routing through SQLAlchemy under the hood.
...
9. backend/db/jobs.py (275 lines -> ~40 lines):
   - Delegate all job + daily stats functions to JobRepository.

10. backend/db/wanted.py (356 lines -> ~40 lines):
    - Delegate all wanted item functions to WantedRepository.

11. backend/db/profiles.py (303 lines -> ~40 lines):
    - Delegate all profile + series/movie mapping functions to ProfileRepository.

12. backend/db/providers.py (368 lines -> ~40 lines):
    - Delegate all provider stats functions to ProviderRepository.

13. backend/db/hooks.py (446 lines -> ~50 lines):
    - Delegate all hook, webhook, and hook_log functions to HookRepository.

14. backend/db/standalone.py (408 lines -> ~50 lines):
    - Delegate all watched folder, series, movie, metadata cache, anidb functions to StandaloneRepository.

CRITICAL: Every public function signature MUST remain identical. Same name, same parameters, same return type. Import paths like `from db.jobs import create_job` must continue working. This is NOT a renaming -- it is a transparent delegation.
...
`cd Z:/CC/Sublarr/backend && python -c "
from app import create_app
app = create_app(testing=True)
with app.app_context():
    from db.config import save_config_entry, get_config_entry
    save_config_entry('test_key', 'test_value')
    assert get_config_entry('test_key') == 'test_value'
    print('Config CRUD via repository: OK')

    from db.jobs import create_job, get_job
    job = create_job('/test/file.mkv')
    fetched = get_job(job['id'])
    assert fetched is not None
    print('Job CRUD via repository: OK')
...
3. CRUD operations work through the new layer (smoke test in Task 2 verify)
